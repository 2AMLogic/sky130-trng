...
> **SUPERSEDED — `ro_array_core` is now a `--check`-reproducible cell recipe
> in [`layout/ro_array_core/`](../ro_array_core/README.md)** (same verdict:
> 132/132 devices, 96/96 nets, 0 DRC violations). Unlike
> `layout/xor2-placement-poc/`, **nothing in this directory is corrected by
> that promotion** — every increment below reproduces, and the promoted
> recipe's `core` stage is `signal6`'s own request, unchanged in substance.
> Three things are worth knowing before reading on:
>
> 1. **Six of the nine `gen-compose` calls below are recipe; three are
>    history.** `signal`…`signal6` were never a chain — each one re-composed
>    the *same* eleven-block floorplan with one more net routed than the
>    increment before it, so only `signal6` is load-bearing. `signal7`/
>    `signal8`/`signal9` *are* a chain and become the promoted cell's
>    `t2bridge`/`vddstub`/`vddbus` stages. The five superseded `signal*` file
>    sets stay here as this directory's append-only record of how the routing
>    was found.
> 2. **The two still-open items this directory's own list called
>    not-a-separate-deliverable are closed there.** `ring1..4`'s and
>    `xa1..3`'s own `vss` taps are drawn (`layout/ro_array_core/`'s
>    `vssstub`/`vssbus` stages, measured first by that directory's own
>    `vss-tap-scan.py`), and the recipe has `compose-cell.py --check`
>    reproducibility. Everything else on that list — array-level parasitic
>    extraction, post-layout PVT, DR-0003 §8's `wstv` decorrelation question,
>    `sampler_core`/`sampler_dff` — is still open (#27).
> 3. **`array-reference.py` is superseded by `cell.json`'s own `lvs` block.**
>    `layout/ro_array_core/`'s reference is generated by `compose-cell.py`'s
>    `lvs.dependency_variants` mechanism from the descriptor, and comes out
>    line-for-line identical to what this script produces.
>    `lvs-negative-controls.py` is promoted too, and runs there against that
>    generated reference. Both stay here, still runnable, as this
>    directory's own evidence.
