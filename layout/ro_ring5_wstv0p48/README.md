# layout/ro_ring5_wstv0p48

The `wstv = 0.48 µm` starve-width instance of `ro_ring5` —
`design/ro_array_core.spice`'s ring `xr4` (`xr1` is
...
python3 layout/bin/compose-cell.py layout/ro_ring5_wstv0p48/cell.json          # rebuild in place
python3 layout/bin/compose-cell.py layout/ro_ring5_wstv0p48/cell.json --check  # verify, don't overwrite
```
...
| `klt extract --deck sky130` | 22 devices (11 nfet + 11 pfet), 19 nets | `extract.json`, `ro_ring5_wstv0p48.spice` |
| `klt lvs` vs. `.subckt ro_ring5` at `wstv=0.48`, `lstv=2` (matching `xr4`) | **match** — 22/22 devices, 19/19 nets, 0 errors | `lvs.json`, `ro_ring5_wstv0p48.ref.spice` |
...
1. the five `blocks[].cell.gds_path`/`cell_name` pairs point at
   `layout/ro_nand2_wstv0p48/` and `layout/ro_stage_wstv0p48/` instead of
   `layout/ro_nand2/` and `layout/ro_stage/`;
2. `lvs.params.wstv: 0.48`;
