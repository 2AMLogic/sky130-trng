# layout/ro_nand2_wstv0p48

The `wstv = 0.48 µm` starve-width instance of `ro_nand2` —
`design/ro_array_core.spice`'s ring `xr4`, the last of the four-ring ladder
(`xr1` is `layout/ro_nand2/`, built at the nominal `wstv = 0.42 µm`). Same
six devices, same eight nets, same floorplan and routing as
`layout/ro_nand2/` — see that cell's own README for the full methodology
...
python3 layout/bin/compose-cell.py layout/ro_nand2_wstv0p48/cell.json          # rebuild in place
python3 layout/bin/compose-cell.py layout/ro_nand2_wstv0p48/cell.json --check  # verify, don't overwrite
```
...
| `klt extract --deck sky130` | 6 devices (3 nfet, 3 pfet), 8 nets | `extract.json`, `ro_nand2_wstv0p48.spice` |
| `klt lvs` vs. `design/ro_array_core.spice`'s own `.subckt ro_nand2` (`wstv=0.48`, `lstv=2` — matching `xg` at `xr4`) | **match** — 6/6 devices, 8/8 nets, 0 mismatches | `lvs.json`, `ro_nand2_wstv0p48.ref.spice` |
...
completes the four-ring `ro_nand2` ladder (`xr1`-`xr4`, `wstv` 0.42-0.48 µm)
— combined with `ro_stage`'s own completed ladder, issue #27 step 2's
"repeat for `ro_stage`, `ro_nand2` (including the 4-way `wstv` variants)"
is now fully closed. `xor2` and hierarchical assembly remain — see
`layout/README.md`.
