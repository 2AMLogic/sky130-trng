...
- **Layout and DRC/LVS.** `layout/` now holds fourteen composed **DRC-clean
  ...
  **Two committed negative controls** (resizing ring 4's starve
  ...
  vacuous. **A later increment (issue #22) closes the block-level
  post-layout PVT gap this section used to name as unattempted**:
  `layout/pex-array/` extracts the above array GDS flat with `klt extract
  ...
  2.158×–2.490× in ring period against pre-layout, the `wstv` ladder still
  discriminates (span 1.089×–1.180×), combining-node bias stays close to
  ...
  wider than the prior ring-scale study (`spec/decision-records/DR-0006-*.md`).
  **This does not close DR-0003 §8**: its first-named mechanism, shared
  ...
  `--check`-reproducible cell recipe (the placement/routing side; this is
  also what `layout/pex-array/` extracts its GDS from — see that
  directory's README for why that is deliberate, not a shortcut taken in
  place of the recipe), a `vddr1`-`vddr4` supply-distribution layout, and
