...
- **Layout and DRC/LVS.** `layout/` now holds fourteen composed **DRC-clean
  ...
  **`ro_array_core` is now `klt drc` clean (0 violations) and `klt lvs`
  ...
  one-shared-`params` mechanism cannot express, so it is generated by a
  small committed script reusing that same rewrite four times over renamed
  copies; **two committed negative controls** (resizing ring 4's starve
  ...
  `--check`-reproducible cell recipe, **the sampler as assembled layout**
